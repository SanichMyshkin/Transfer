...
# SQLite settings
SQLITE_FILE = os.getenv("SQLITE_FILE", "victoria.db")
...
def discover_and_route_series(
        ...
        unacc_rows.append(
            {
                ...
                "service_name": clean_spaces(service_name),
                "activity_code": clean_spaces(activity_code),
                "activity_name": clean_spaces(activity_name),
                "metric": metric or "",
                "samples_value": int(samples_value or 0),
                "reason": reason,
                "detail": detail,
            }
        )
    ...
    for idx, metric_name in enumerate(metric_names, 1):
        ...
        if not rows and VERBOSE_LOG:
            log.info("RESULT | metric=%s | no_series", metric_name)
        ...
        for r in rows or []:
            ...
            service_id_final = pick_better_sid(service_id_raw, sid_from_team)

            ov = overrides.get(team_base)
            if ov:
                service_id_final = ov

            activity = activity_map.get(service_id_final, {}) if service_id_final else {}
            ...
            elif service_id_final and service_id_final in ban_service_set:
                status = "unaccounted"
                stage = "discover"
                reason = "banned_service_id"
                detail = "service_id in BAN_SERVICE_IDS"
            elif not service_id_final:
                status = "unaccounted"
                stage = "discover"
            ...
            if status == "unaccounted":
                add_unacc(
                    ...
                    service_name=service_name or team_base,
                ...
                if VERBOSE_LOG:
                    log.info(
                        "ROUTE | unaccounted | metric=%s | team=%s | sid=%s | samples=%s | reason=%s",
                        metric,
                        team_base or "-",
                        service_id_final or "-",
                        samples_value,
                ...
                key = (team_base, service_id_final, activity_code)
                ...
                if VERBOSE_LOG:
                    log.info(
                        "ROUTE | report | metric=%s | team=%s | sid=%s | samples=%s",
                        metric,
                        team_base or "-",
                        service_id_final or "-",
                        samples_value,
    ...
    df_unacc = pd.DataFrame(unacc_rows, columns=UNACC_COLS)
    log.info(
...
def enrich_group_rows(group_rows):
    if not group_rows:
        return pd.DataFrame(
            columns=[
                "team",
...
# ---------------------------------------------------------------------------
# SQLite database layer (replaces PostgreSQL / psycopg2)
# ---------------------------------------------------------------------------
...
class SqliteDb:
    ...
    def init_schema(self):
        with self.conn:
            self.conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_das_snapshot_date
                    ON daily_activity_samples (snapshot_date);

                CREATE INDEX IF NOT EXISTS idx_das_service_id
                ...
                CREATE INDEX IF NOT EXISTS idx_du_snapshot_date
    ...
    def save_daily_snapshot(self, snapshot_date: str, df_daily: pd.DataFrame):
        ...
        with self.conn:
            ...
            if rows:
                self.conn.executemany(
                    ...
                    INSERT OR IGNORE INTO daily_activity_samples (
                        snapshot_date, service_key, service_id, service_name,
                        activity_code, activity_name, samples_value, window_hours, created_at
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    rows,
            ...
            self.conn.execute(
                """
                INSERT OR IGNORE INTO snapshot_runs (snapshot_date, rows_count, created_at)
                VALUES (?, ?, ?)
                """,
    ...
    def load_period_rows(self, lookback_days: int) -> pd.DataFrame:
        cols = [
            "snapshot_date", "service_id", "service_name",
            "activity_code", "activity_name", "samples_value", "window_hours",
        ]

        if lookback_days <= 0:
            cur = self.conn.execute(
                """
                SELECT snapshot_date, service_id, service_name,
                       activity_code, activity_name, samples_value, window_hours
        ...
        with self.conn:
            ...
            if df_unacc is not None and not df_unacc.empty:
                ...
                for row in df_unacc.to_dict("records"):
                    sv = row.get("samples_value", "")
                    samples_value_str = "" if sv == "" else str(sv)
            ...
            if rows:
                self.conn.executemany(
                    ...
                    INSERT INTO daily_unaccounted (
                        snapshot_date, stage, team, service_id, service_name,
                        activity_code, activity_name, metric, samples_value,
    ...
    def load_daily_unaccounted(self, snapshot_date: str) -> pd.DataFrame:
        cur = self.conn.execute(
            """
            SELECT stage, team, service_id, service_name,
                   activity_code, activity_name, metric, samples_value, reason, detail
...
# ---------------------------------------------------------------------------
# Period report helpers (unchanged)
# ---------------------------------------------------------------------------
...
def build_period_report(df_period: pd.DataFrame) -> pd.DataFrame:
    ...
    if not has_code.empty:
        has_code = has_code.groupby(
            ["Код", "Код активности"], as_index=False,
        ).agg(
            {
                "Имя сервиса":              _first_non_empty,
                "Наименование активности":  _first_non_empty,
        ...
        out_parts.append(
            has_code.rename(columns={"samples_value": samples_col})[
                ["Имя сервиса", "Код", "Код активности", "Наименование активности", samples_col]
            ]
        )

    if not no_code.empty:
        no_code = no_code.groupby(
            ["Имя сервиса", "Код активности"], as_index=False,
        ).agg(
            {
                "Код":                     _first_non_empty,
                "Наименование активности": _first_non_empty,
...
def main():
    ...
    try:
        ...
        if db.has_snapshot(today_snapshot):
            ...
            enriched = enrich_group_rows(group_rows)
            log.info("Enrich done | rows=%d", len(enriched))

            if not enriched.empty:
                df_daily = build_daily_df_report(enriched)
            else:
