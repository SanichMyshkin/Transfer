...
def main():
    ...
    triggerids_all, host_trigger_ids, host_tags_found = fetch_triggers_by_tags(api, hostids)
    logger.info(f"Уникальных триггеров по всем тегам: {len(triggerids_all)}")

    if triggerids_all:
        host_events = fetch_events_count_by_host(api, triggerids_all, time_from, time_till)
    else:
        host_events = {}
    ...
    for r in df_tagged_hosts.to_dict("records"):
        ...
        if len(trig_ids) > MAX_TRIGGERIDS_IN_CELL:
            trig_ids_cell += f",...(+{len(trig_ids) - MAX_TRIGGERIDS_IN_CELL})"
