...
def resolve_business_type_by_maintainers(project_name, maintainers, bk_map):
    found = []
    ...
    log.info(f'BT project="{project_name}" source=maintainers chosen="{bt}" found={found} counts={counts}')
    return bt
...
def main():
    ...
    for i, p in enumerate(gl.projects.list(all=True, iterator=True), start=1):
        ...
        try:
            ...
            bt = clean_spaces(resolve_business_type_by_maintainers(proj_name, maintainers_unique, bk_map))

            if bt in BAN_BUSINESS_TYPE:
                log.info(f'PROJECT skip (banlist) name="{proj_name}" bt="{bt}" maintainers="{maintainers_str}"')
                continue
            ...
            log.info(
                f'PROJECT map name="{proj_name}" bt="{bt}" maintainers="{maintainers_str}" '
