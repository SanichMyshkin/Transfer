    ...
    for r in runners:
        ...
        try:
            ...
            if full_runner.runner_type == "group_type":
                groups = getattr(full_runner, "groups", [])
                if isinstance(groups, list) and groups:
                    g = groups[0]
                    path = g.get("full_path", "") or g.get("web_url", "")
                    name = g.get("name", "")

            # --- если раннер привязан к проекту ---
            elif full_runner.runner_type == "project_type":
                projects = getattr(full_runner, "projects", [])
                if isinstance(projects, list) and projects:
                    p = projects[0]
                    path = p.get("path_with_namespace", "") or p.get("web_url", "")
                    name = p.get("name", "")
