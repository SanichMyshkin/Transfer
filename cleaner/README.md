...
- `requests` – HTTP-запросы к API Nexus.
- `PyYAML` – работа с YAML-конфигами.
- `python-dateutil` – парсинг и работа с датами (lastModified, lastDownloaded).
- `python-dotenv` – загрузка `.env` с кредами (логин, пароль, BASE_URL).
- `urllib3` – для управления SSL-предупреждениями.
...
## ⚙️ Принцип работы
1. Скрипт ищет все `.yaml` файлы в `configs/`.
2. Загружает их содержимое.
3. Для каждого репозитория:
   - Узнаёт его тип (`raw`, `docker`, `maven2`).
   - Забирает список артефактов.
   - Сравнивает с правилами.
   - Отбирает кандидатов на удаление.
   - Удаляет (или логирует, если `dry_run: true`).
...
> Поддерживаются репозитории форматов: **`docker`** и **`raw`**
...
| Поле                                    | Описание                                                                |
| --------------------------------------- | ----------------------------------------------------------------------- |
| `repo_names`                            | Список репозиториев (`docker` или `raw`), в которых будет производиться очистка |
...
Скрипт поддерживает **две схемы фильтрации** в зависимости от типа репозитория:
...
> Важно: структура именования компонентов в RAW может быть произвольной, но **фильтрация всегда ориентируется на имя файла**, а не путь.
