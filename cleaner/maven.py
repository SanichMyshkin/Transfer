...
SUPPORTED_FORMATS = ["raw", "docker", "maven2"]
...
def get_repository_items(repo_name, repo_format):
    """
    Получает список элементов из репозитория в зависимости от формата.
    raw  → /assets
    docker, maven2 → /components
    """
    items = []
    continuation_token = None
    endpoint = "assets" if repo_format == "raw" else "components"
    ...
    while True:
        ...
        try:
            response = requests.get(
                f"{BASE_URL}service/rest/v1/{endpoint}",
...
def convert_raw_assets_to_components(assets):
    """
    Преобразует список assets формата raw в компоненты.
    """
    components = []
    for asset in assets:
        path = asset.get("path", "")
...
def normalize_maven_snapshot_versions(components):
    """
    Для Maven SNAPSHOT версий можно добавить логику фильтрации timestamp-билдов.
...
def delete_component(component_id, component_name, component_version, dry_run, use_asset=False):
    if dry_run:
        logging.info(f"[DELETE] 🧪 [DRY_RUN] Пропущено удаление: {component_name}:{component_version} (ID: {component_id})")
        return
    ...
    try:
        ...
        response.raise_for_status()
        logging.info(f"[DELETE] ✅ Удалён: {component_name}:{component_version} (ID: {component_id})")
    except requests.exceptions.HTTPError as e:
        if response.status_code == 404:
            logging.warning(f"[DELETE] ⚠️ Компонент не найден (404): {component_name}:{component_version} (ID: {component_id})")
        else:
...
def get_matching_rule(version, regex_rules, no_match_retention, no_match_reserved, no_match_min_days_since_last_download):
    ...
    if matched_rules:
        ...
        retention = timedelta(days=retention_days) if retention_days is not None else None
        return pattern, retention, reserved, min_days_since_last_download
    retention = timedelta(days=no_match_retention) if no_match_retention is not None else None
...
def filter_components_to_delete(components, regex_rules, no_match_retention, no_match_reserved, no_match_min_days_since_last_download):
    ...
    for component in components:
        ...
        if not last_modified_strs:
            logging.info(f" ⏭ Пропуск: отсутствует lastModified у {name}:{version}")
            continue
    ...
    for (name, pattern), group in grouped.items():
        ...
        for i, component in enumerate(sorted_group):
            ...
            if passed_retention or passed_recent_download:
                logging.info(f" 📦 Сохранён: {full_path} | правило ({pattern})")
                continue
            else:
                logging.info(f" 🗑 Удаление: {full_path} | правило ({pattern})")
...
def clear_repository(repo_name, cfg):
    ...
    if not repo_format:
        logging.warning(f"⚠️ Пропущен '{repo_name}' — неизвестный формат")
        return

    if repo_format not in SUPPORTED_FORMATS:
    ...
    if not items:
        logging.info(f"ℹ️ '{repo_name}' пуст")
        return
    ...
    if repo_format == "maven2":
        components = normalize_maven_snapshot_versions(components)
    ...
    to_delete = filter_components_to_delete(
        ...
        no_match_min_days_since_last_download=cfg.get("no_match_min_days_since_last_download", None),
    )
...
def main():
    ...
    if not config_files:
        logging.warning("[MAIN] ⚠️ В 'configs/' нет YAML-файлов")
        return
    for cfg_path in config_files:
        logging.info(f"\n📄 Обработка файла: {cfg_path}")
