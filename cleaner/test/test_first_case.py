...
def make_component(name, version, last_modified_days_ago, last_download_days_ago=None):
    """Создаёт минимальный компонент для теста"""
...
# ======== TEST 1: repository.py ========
def test_repository_all_rules():
    components = [
        make_component(
    ...
    # Проверка: top-2 по last_modified reserved остаются
    reserved_versions = [c["version"] for c in components if not c["will_delete"]]

    # Сортировка по last_modified: dev-3 (1 день), dev-1 (2 дня), dev-2 (6 дней)
    assert set(reserved_versions) == {"dev-3", "dev-1"}


# ======== TEST 2: maven.py ========
def test_maven_all_rules():
    components = [
        {
            "group": "org.test",
            "name": "libA",
    ...
    # Top-2 reserved → 1.0.2 и 1.0.0 сохраняются
