...
# ======== TEST: repository.py ========
def test_repository_second_case():
    components = [
        make_component("compA", "dev-1", last_modified_days_ago=2, last_download_days_ago=1),
        make_component("compA", "dev-2", last_modified_days_ago=6, last_download_days_ago=10),
        make_component("compA", "dev-3", last_modified_days_ago=1, last_download_days_ago=0),
    ]
...
# ======== TEST: maven.py ========
def test_maven_second_case():
    components = [
        {"group": "org.test", "name": "libA", "version": "1.0.0", "assets": [
            {"lastModified": (datetime.now(timezone.utc) - timedelta(days=2)).isoformat(),
             "lastDownloaded": (datetime.now(timezone.utc) - timedelta(days=1)).isoformat()}
        ]},
        {"group": "org.test", "name": "libA", "version": "1.0.1", "assets": [
            {"lastModified": (datetime.now(timezone.utc) - timedelta(days=6)).isoformat(),
             "lastDownloaded": (datetime.now(timezone.utc) - timedelta(days=10)).isoformat()}
        ]},
        {"group": "org.test", "name": "libA", "version": "1.0.2", "assets": [
            {"lastModified": (datetime.now(timezone.utc) - timedelta(days=1)).isoformat(),
             "lastDownloaded": (datetime.now(timezone.utc) - timedelta(days=0)).isoformat()}
        ]},
    ]
    ...
    maven_rules = {
        "release": {
            "regex_rules": {
                ".*": {"retention_days": 5, "reserved": 2, "min_days_since_last_download": 3}
            },
