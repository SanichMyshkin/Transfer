...
# ======== TEST CASE 20: REPOSITORY ========
def test_repository_case20():
    components = [
        make_component(
        ...
        ),  # NO-MATCH → age<no_match_retention → сохраняем
        make_component(
            "compB", "random-2", last_modified_days_ago=10
        ),  # NO-MATCH → age>no_match_retention → удалится
    ]
    ...
    assert set(deleted_versions) == {"dev-3", "random-2"}


# ======== TEST CASE 20: MAVEN ========
def test_maven_case20():
    now = datetime.now(timezone.utc)
    components = [
        {
            "group": "org.test",
    ...
    assert set(deleted_versions) == {"1.0.2", "2.0.1"}
