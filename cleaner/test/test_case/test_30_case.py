...
# ======== TEST CASE 30: REPOSITORY ========
def test_repository_case30():
    components = [
        # MATCH
        make_component(
        ...
        ),  # без retention/min_days → удаляется
        make_component("compB", "random-3", last_modified_days_ago=12),  # удаляется
    ]
...
def test_maven_case30():
    ...
    components = [
        ...
        },  # без retention/min_days → удаляется
        {
            ...
            "name": "libB",
            "version": "2.0.2",
            "assets": [{"lastModified": (now - timedelta(days=12)).isoformat()}],
        },  # удаляется
    ]
