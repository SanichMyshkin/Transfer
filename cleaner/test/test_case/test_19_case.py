...
# ======== TEST CASE 19: REPOSITORY ========
def test_repository_case19():
    components = [
        make_component(
        ...
        make_component(
            "compB", "random-1", last_modified_days_ago=1, last_download_days_ago=0
        ),  # NO-MATCH → сохраняем
        make_component(
            "compB", "random-2", last_modified_days_ago=6, last_download_days_ago=2
        ),  # age<no_match_retention → сохраняем
        make_component(
            "compB", "random-3", last_modified_days_ago=10, last_download_days_ago=5
        ),  # удалится: age>no_match_retention, dl>no_match_min_days
    ]
    ...
    to_delete = filter_components_to_delete(
        ...
        no_match_reserved=None,  # NO-MATCH без reserved
            ...
            "assets": [
                {
                    ...
                    "lastDownloaded": (now - timedelta(days=0)).isoformat(),
                }
            ],
        },  # NO-MATCH → сохраняем
        {
