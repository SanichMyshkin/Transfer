...
def test_repository_case55():
    components = [
        ...
        # NO-MATCH: только min_days
        make_component("compB", "random-1", 10, 2),   # dl=2 <= 7 → сохраняем
        make_component("compB", "random-2", 10, 20),  # dl=20 > 7 → удаляется
    ]
    ...
    to_delete = filter_components_to_delete(
        ...
        no_match_min_days_since_last_download=7,  # ✅ min_days включён
    )
...
def test_maven_case55():
    now = datetime.now(timezone.utc)
    components = [
        # MATCH
        {"group": "org.test", "name": "libA", "version": "1.0.0",
         "assets": [{"lastModified": (now - timedelta(days=10)).isoformat(),
                     "lastDownloaded": (now - timedelta(days=2)).isoformat()}]},   # dl=2 <= 5 → сохраняем
        {"group": "org.test", "name": "libA", "version": "1.0.1",
         "assets": [{"lastModified": (now - timedelta(days=10)).isoformat(),
                     "lastDownloaded": (now - timedelta(days=20)).isoformat()}]},  # dl=20 > 5 → удаляется

        # NO-MATCH
        {"group": "org.test", "name": "libB", "version": "2.0.0",
         "assets": [{"lastModified": (now - timedelta(days=10)).isoformat(),
                     "lastDownloaded": (now - timedelta(days=2)).isoformat()}]},   # dl=2 <= 7 → сохраняем
        {"group": "org.test", "name": "libB", "version": "2.0.1",
         "assets": [{"lastModified": (now - timedelta(days=10)).isoformat(),
                     "lastDownloaded": (now - timedelta(days=20)).isoformat()}]},  # dl=20 > 7 → удаляется
    ]
