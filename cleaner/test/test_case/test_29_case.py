...
# ======== TEST CASE 29: REPOSITORY ========
def test_repository_case29():
    components = [
        # MATCH
        make_component(
        ...
        # NO-MATCH
        make_component(
            "compB", "random-1", last_modified_days_ago=5, last_download_days_ago=1
        ),  # top reserved → сохраняем
        make_component(
            "compB", "random-2", last_modified_days_ago=10, last_download_days_ago=2
        ),  # dl<min_days → сохраняем
        make_component(
            "compB", "random-3", last_modified_days_ago=12, last_download_days_ago=10
        ),  # удаляется
    ]
