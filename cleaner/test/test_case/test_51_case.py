...
# ===================== CASE 51 =====================
def test_repository_case51():
    components = [
        make_component("compA", "dev-1", 10, 2),  # match, dl=2 <= 5 → сохраняем
        make_component("compA", "dev-2", 10, 20),  # match, dl=20 > 5 → удаляется
        make_component("compB", "random-1", 3),  # no-match, age=3 <= 7 → сохраняем
        make_component("compB", "random-2", 15, 2),  # no-match, dl=2 <= 5 → сохраняем
        make_component(
            "compB", "random-3", 20, 30
        ),  # no-match, age>7 и dl>5 → удаляется
    ]
