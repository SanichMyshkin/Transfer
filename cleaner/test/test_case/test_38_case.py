...
# ======== TEST CASE 38: REPOSITORY ========
def test_repository_case38():
    components = [
        # MATCH
        make_component(
        ...
        ),  # top no_match_reserved → сохраняем
        make_component("compB", "random-2", last_modified_days_ago=2),  # удаляется
        make_component("compB", "random-3", last_modified_days_ago=3),  # удаляется
    ]
