...
# ======== TEST CASE 44: REPOSITORY ========
def test_repository_case44():
    components = [
        # MATCH
        ...
        # NO-MATCH
        make_component("compB", "random-1", last_modified_days_ago=3),  # age < retention → сохраняем
        make_component("compB", "random-2", last_modified_days_ago=10), # age > retention → удаляется
    ]
    ...
    assert set(deleted_versions) == {"dev-2", "dev-3", "random-2"}


# ======== TEST CASE 44: MAVEN ========
def test_maven_case44():
    now = datetime.now(timezone.utc)
    components = [
        # MATCH
        {
    ...
    assert set(deleted_versions) == {"1.0.1", "1.0.2", "2.0.1"}
