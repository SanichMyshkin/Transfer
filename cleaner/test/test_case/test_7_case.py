...
# ======== TEST: repository.py ========
def test_repository_seventh_case():
    components = [
        make_component(
    ...
    # Match: Top-2 reserved → dev-3 и dev-1
    # No-match: сохранение только если last_download ≤ 7
    assert set(saved_versions) == {"dev-1", "dev-3"}


# ======== TEST: maven.py ========
def test_maven_seventh_case():
    components = [
        {
            "group": "org.test",
            "name": "libA",
    ...
    # Match: Top-2 reserved → 1.0.2 и 1.0.0
    # No-match: сохраняем только если last_download ≤ 7 → 1.0.1 удаляется
    assert set(saved_versions) == {"1.0.0", "1.0.2"}
