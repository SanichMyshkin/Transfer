...
# ======== TEST CASE 47: REPOSITORY ========
def test_repository_case47():
    components = [
        # MATCH
        make_component("compA", "dev-1", last_modified_days_ago=1),  # reserved → сохраняем
        make_component("compA", "dev-2", last_modified_days_ago=5),  # удаляется
        make_component("compA", "dev-3", last_modified_days_ago=10), # удаляется
        # NO-MATCH
        make_component("compB", "random-1", last_modified_days_ago=1, last_download_days_ago=2),  # dl < min_days → сохраняем
        make_component("compB", "random-2", last_modified_days_ago=5, last_download_days_ago=6),  # dl > min_days → удаляется
        make_component("compB", "random-3", last_modified_days_ago=10, last_download_days_ago=10),# dl > min_days → удаляется
    ]
    ...
    assert set(deleted_versions) == {"dev-2", "dev-3", "random-2", "random-3"}


# ======== TEST CASE 47: MAVEN ========
def test_maven_case47():
    now = datetime.now(timezone.utc)
    components = [
        # MATCH
        {
        ...
        # NO-MATCH
        {
            ...
            "version": "2.0.0",
            "assets": [
                {"lastModified": (now - timedelta(days=1)).isoformat(), "lastDownloaded": (now - timedelta(days=2)).isoformat()}
            ],
        },  # dl < min_days → сохраняем
        {
            ...
            "version": "2.0.1",
            "assets": [
                {"lastModified": (now - timedelta(days=5)).isoformat(), "lastDownloaded": (now - timedelta(days=6)).isoformat()}
            ],
        },  # dl > min_days → удаляется
        {
            ...
            "version": "2.0.2",
            "assets": [
                {"lastModified": (now - timedelta(days=10)).isoformat(), "lastDownloaded": (now - timedelta(days=10)).isoformat()}
            ],
        },  # dl > min_days → удаляется
    ]
    ...
    assert set(deleted_versions) == {"1.0.1", "1.0.2", "2.0.1", "2.0.2"}
