...
# ======== TEST CASE 40: REPOSITORY ========
def test_repository_case40():
    components = [
        # MATCH
        make_component(
        ...
        # NO-MATCH (все сохраняются)
        make_component("compB", "random-1", last_modified_days_ago=1),
        make_component("compB", "random-2", last_modified_days_ago=2),
        make_component("compB", "random-3", last_modified_days_ago=3),
    ]
    ...
    assert set(deleted_versions) == {"dev-3", "dev-4"}


# ======== TEST CASE 40: MAVEN ========
def test_maven_case40():
    now = datetime.now(timezone.utc)
    components = [
        # MATCH
        {
    ...
    assert set(deleted_versions) == {"1.0.2", "1.0.3"}
