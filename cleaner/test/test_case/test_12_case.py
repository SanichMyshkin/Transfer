...
def make_maven_component(
    group, name, version, last_modified_days_ago, last_download_days_ago=None
):
    now = datetime.now(timezone.utc)
    last_modified = now - timedelta(days=last_modified_days_ago)
    asset = {"lastModified": last_modified.isoformat()}
...
# ======== TEST CASE 12: NO-MATCH (repository) ========
...
# ======== TEST CASE 12: MATCH (maven) ========
def test_maven_case12_match():
    components = [
        make_maven_component(
            "org.test", "libA", "1.0.0", last_modified_days_ago=1
        ),  # reserved
        make_maven_component(
            "org.test", "libA", "1.0.1", last_modified_days_ago=2
        ),  # reserved
        make_maven_component(
            "org.test", "libA", "1.0.2", last_modified_days_ago=5
        ),  # age=5 <= 7 → сохраняем
        make_maven_component(
            "org.test", "libA", "1.0.3", last_modified_days_ago=12
        ),  # age=12 > 7 → удаляем
    ]
...
# ======== TEST CASE 12: NO-MATCH (maven) ========
def test_maven_case12_no_match():
    components = [
        make_maven_component(
            "org.test", "libB", "2.0.0", last_modified_days_ago=2
        ),  # age=2 <= 5 → сохраняем
        make_maven_component(
            "org.test", "libB", "2.0.1", last_modified_days_ago=8
        ),  # age=8 > 5 → удаляем
    ]
    ...
    maven_rules = {
        "release": {
            "regex_rules": {
                "^1\\.0\\.": {"retention_days": 7, "reserved": 2}
            },  # не подходит для 2.*
            "no_match_retention_days": 5,
            "no_match_reserved": None,
            "no_match_min_days_since_last_download": None,
    ...
    assert set(deleted_versions) == {"2.0.1"}
