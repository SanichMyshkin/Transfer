...
# ===================== CASE 59 =====================
def test_repository_case59():
    components = [
        # MATCH отсутствует (нет правил)
        make_component("compA", "dev-1", 5, 2),
        make_component("compA", "dev-2", 10, 20),

        # NO-MATCH: reserved отсутствует, retention=7, min_days=5
        make_component("compB", "random-1", 1, 30),   # dl>5, age<7 → сохраняем
        make_component("compB", "random-2", 5, 2),    # dl<5, age<7 → сохраняем
        make_component("compB", "random-3", 20, 10),  # dl>5, age>7 → удаляется
    ]
    ...
    components = [
        ...
        {"group": "org.test", "name": "libB", "version": "2.0.0",
         "assets": [{"lastModified": (now - timedelta(days=1)).isoformat(),
                     "lastDownloaded": (now - timedelta(days=30)).isoformat()}]},  # age<7 → сохраняем
        ...
        {"group": "org.test", "name": "libB", "version": "2.0.2",
         "assets": [{"lastModified": (now - timedelta(days=20)).isoformat(),
                     "lastDownloaded": (now - timedelta(days=10)).isoformat()}]},  # age>7, dl>5 → удаляется
    ]
