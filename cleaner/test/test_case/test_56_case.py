...
def test_repository_case56():
    components = [
        ...
        # NO-MATCH: без любых правил → все сохраняются
        make_component("compB", "random-1", 10, 2),
        make_component("compB", "random-2", 10, 20),
        make_component("compB", "random-3", 10, 30),
    ]
    ...
    assert set(saved) == {"dev-1", "random-1", "random-2", "random-3"}
    assert set(deleted) == {"dev-2"}


def test_maven_case56():
    now = datetime.now(timezone.utc)
    components = [
        # MATCH
        {
    ...
    assert set(saved) == {"1.0.0", "2.0.0", "2.0.1", "2.0.2"}
    assert set(deleted) == {"1.0.1"}
