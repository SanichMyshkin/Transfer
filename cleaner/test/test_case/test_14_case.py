...
# ======== TEST CASE 14: MATCH (repository) ========
def test_repository_case14_match():
    components = [
...
# ======== TEST CASE 14: NO-MATCH (repository) ========
def test_repository_case14_no_match():
    components = [
        make_component("compB", "random-1", last_modified_days_ago=1),   # reserved
        make_component("compB", "random-2", last_modified_days_ago=2),   # удалится
    ...
    assert set(deleted_versions) == {"random-2", "random-3"}


# ======== TEST CASE 14: MATCH (maven) ========
def test_maven_case14_match():
    components = [
        ...
        make_maven_component("org.test", "libA", "1.0.2", last_modified_days_ago=5),   # age=5 < 7 → сохраняем
        make_maven_component("org.test", "libA", "1.0.3", last_modified_days_ago=12),  # age=12 > 7 → удаляем
    ]
...
# ======== TEST CASE 14: NO-MATCH (maven) ========
def test_maven_case14_no_match():
    components = [
        make_maven_component("org.test", "libB", "2.0.0", last_modified_days_ago=1),   # reserved
        make_maven_component("org.test", "libB", "2.0.1", last_modified_days_ago=2),   # удалится
        make_maven_component("org.test", "libB", "2.0.2", last_modified_days_ago=10),  # удалится
    ]

    maven_rules = {
        "release": {
            "regex_rules": {"^1\\.0\\.": {"retention_days": 7, "reserved": 2}},  # не подходит для 2.*
            "no_match_retention_days": None,  # ❌
            "no_match_reserved": 1,           # ✅
            "no_match_min_days_since_last_download": None,  # ❌
        }
    ...
    assert set(saved_versions) == {"2.0.0"}
    assert set(deleted_versions) == {"2.0.1", "2.0.2"}
