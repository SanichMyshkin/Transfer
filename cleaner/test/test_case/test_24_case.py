...
# ======== TEST CASE 24: REPOSITORY ========
def test_repository_case24():
    components = [
        make_component(
        ...
        ),  # NO-MATCH, нет правил → сохраняется
        make_component(
            "compB", "random-2", last_modified_days_ago=6
        ),  # NO-MATCH, нет правил → сохраняется
        make_component(
            "compB", "random-3", last_modified_days_ago=10
        ),  # NO-MATCH, нет правил → сохраняется
    ]
    ...
    # MATCH: dev-1, dev-2 сохраняются, dev-3 удаляется
    # NO-MATCH: все сохраняются, так как нет правил
    assert set(saved_versions) == {"dev-1", "dev-2", "random-1", "random-2", "random-3"}
    assert set(deleted_versions) == {"dev-3"}


# ======== TEST CASE 24: MAVEN ========
def test_maven_case24():
    now = datetime.now(timezone.utc)
    components = [
        {
            "group": "org.test",
        ...
        },  # NO-MATCH → сохраняется
        {
            "group": "org.test",
            "name": "libB",
            "version": "2.0.1",
            "assets": [{"lastModified": (now - timedelta(days=6)).isoformat()}],
        },  # NO-MATCH → сохраняется
        {
            "group": "org.test",
            "name": "libB",
    ...
    assert set(saved_versions) == {"1.0.0", "1.0.1", "2.0.0", "2.0.1", "2.0.2"}
    assert set(deleted_versions) == {"1.0.2"}
