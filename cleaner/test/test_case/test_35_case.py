...
# ======== TEST CASE 35: REPOSITORY ========
def test_repository_case35():
    components = [
        # MATCH (reserved=1, retention=7, min_days=5)
        make_component("compA", "dev-1", last_modified_days_ago=20, last_download_days_ago=20),
        make_component("compA", "dev-2", last_modified_days_ago=3, last_download_days_ago=20),   # reserved (свежий)
        make_component("compA", "dev-3", last_modified_days_ago=20, last_download_days_ago=2),   # dl<5 → сохраняем
        make_component("compA", "dev-4", last_modified_days_ago=20, last_download_days_ago=20),  # удаляется

        # NO-MATCH (retention=7, min_days=5, без reserved)
        make_component("compB", "random-1", last_modified_days_ago=20, last_download_days_ago=20),  # age>7, dl>5 → удаляем
        make_component("compB", "random-2", last_modified_days_ago=3, last_download_days_ago=20),   # age<7 → сохраняем
        make_component("compB", "random-3", last_modified_days_ago=20, last_download_days_ago=2),   # dl<5 → сохраняем
        make_component("compB", "random-4", last_modified_days_ago=20, last_download_days_ago=20),  # age>7, dl>5 → удаляем
    ]
...
def test_maven_case35():
    ...
    # NO-MATCH: 2.0.1 (age<7), 2.0.2 (dl<5)
