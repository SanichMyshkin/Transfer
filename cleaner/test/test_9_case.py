...
def test_repository_ninth_case():
    ...
    regex_rules = {
        "^dev-": {"retention_days": 5, "reserved": 1}
    }
...
def test_maven_ninth_case():
    components = [
        {"group": "org.test", "name": "libX", "version": "1.0.0", "assets": [
            {"lastModified": (datetime.now(timezone.utc) - timedelta(days=2)).isoformat()}
        ]},
        {"group": "org.test", "name": "libX", "version": "1.0.1", "assets": [
            {"lastModified": (datetime.now(timezone.utc) - timedelta(days=6)).isoformat()}
        ]},
        {"group": "org.test", "name": "libX", "version": "1.0.2", "assets": [
            {"lastModified": (datetime.now(timezone.utc) - timedelta(days=10)).isoformat()}
        ]},
    ]
...
def test_repository_case9_match():
    components = [
        ...
        make_component("compA", "dev-2", last_modified_days_ago=10),  # старый, должен удалиться
        make_component("compA", "dev-3", last_modified_days_ago=4),   # свежий, попадет в reserved
    ]
...
def test_repository_case9_no_match():
    components = [
        make_component("compB", "random-1", last_modified_days_ago=1),   # сохранится по reserved
        make_component("compB", "random-2", last_modified_days_ago=3),   # свежий → retention=5
        make_component("compB", "random-3", last_modified_days_ago=10, last_download_days_ago=2),  # сохранится по min_days=3
        make_component("compB", "random-4", last_modified_days_ago=15, last_download_days_ago=20), # старый → удалится
    ]

    regex_rules = {
        "^dev-": {"retention_days": 7, "reserved": 2}  # не сработает, т.к. random-* не матчится
    }
